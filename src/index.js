...
function runUrl(userSearch) {
    ...
    const openWeatherApi = `https://api.openweathermap.org/data/2.5/forecast?q=${searchedLocation}&appid=${API_KEY}`;
    const weatherLocation = document.getElementById('weather_location'); 
    const daysDate = ['date0'];    
    ...
    let promise = fetch(openWeatherApi)
                    .then(response =>{
                        if(!response.ok){
                            loader.style.display = 'block';
                            loader.style.background = '#f1f1f1';
                            beforeSearch.style.display ='none';
...
function displayData(myRes){
    ...
    for(let i =0; i<5; i++){
        ...
        document.getElementById(iconArr[i]).src = `https://openweathermap.org/img/wn/${myRes[i].weather[0].icon}@2x.png`;
